...
class segnet(nn.Module):
    def __init__(self, out_channel=10):
        ...
        self.conv5 = nn.Conv2d(16, out_channel, kernel_size=3, padding=1)
        self.bat1 = nn.BatchNorm2d(16)
        self.bat2 = nn.BatchNorm2d(out_channel)
        #self.bat3 = nn.BatchNorm2d(128)
        self.relu = nn.ReLU()

    def forward(self, x):
        out = self.relu( self.bat1( self.conv1(x) ) )
        out = self.relu( self.bat1( self.conv2(out) ) )
        out = self.relu( self.bat1( self.conv3(out) ) )
        out = self.relu( self.bat1( self.conv4(out) ) )
        out = self.conv5(out)
        return out
...
if __name__ == "__main__":
    batch = torch.zeros(64, 3, 512, 512)
    model = segnet()
    output = model(batch)
    print(output.size())